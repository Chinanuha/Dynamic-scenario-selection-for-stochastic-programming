function [R_new,pi_new,NIter,functime,cvxtime,SumCri1,SumCri2] = AdaModAlgo(Info,S,NIter_RRF,N_interval,R_old,pi_old)
...
while 1
    ...
    for i = 1:S
        ...
        if ismember(pos(i),CR)
           R_temp(i,:) = Info.retm(pos(i),:);
        end
        if ismember(pos(i),NR) 
           R_temp(i,:) = Info.retm(CR(randi([1,size(CR,1)],1,1)),:);
        end
        if ismember(pos(i),intersect(WR,CNR))
           WR_CR = intersect(WR,CR);
           R_temp(i,:) = Info.retm(WR_CR(randi([1,size(WR_CR,1)],1,1)),:);
        end
    end

    % Assigning Probabilities
    sf_temp = Info.L-Info.B*R_temp*x ;
    Interval = linspace(0,max(sf),N_interval+1);
    n_interval = zeros(N_interval,1);
    for k = 1:N_interval
        D(k) = size(find(  sf > Interval(k) &  sf <= Interval(k+1) ),1);
        n_interval(k) = size(find(  sf_temp > Interval(k) &  sf_temp <= Interval(k+1) ),1);
    end
    
    W = max(sf)/N_interval;
    for i = 1:S
        for k = 1:N_interval
            if sf_temp(i) > (k-1)*W && sf_temp(i) <= k*W
               pi_temp(i) = (D(k)/N)/n_interval(k);
            end  
        end
    end
